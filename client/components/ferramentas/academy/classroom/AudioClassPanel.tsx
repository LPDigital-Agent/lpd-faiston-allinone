...
export function AudioClassPanel({ episodeId }: AudioClassPanelProps) {
  ...
  const {
    ...
    formatTime,
    modeLabel,
    setExternalAudioData,
    savedAudioMeta,
    hasSavedAudio,
    loadSavedAudio,
    loadingSavedAudio,
  } = useAudioClass(courseId, episodeId, setAudioData);

  useEffect(() => {
    if (audioData && setExternalAudioData) {
      setExternalAudioData(audioData);
    }
  }, [audioData, setExternalAudioData]);
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-[260px_1fr] gap-5">
              ...
              <div className="space-y-3">
                ...
                {hasSavedAudio && savedAudioMeta && (
                  <div className="p-4 rounded-xl bg-white/[0.03] border border-white/10">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-[var(--faiston-magenta-mid,#C31B8C)]/20 flex items-center justify-center">
                        <Headphones className="w-5 h-5 text-[var(--faiston-magenta-mid,#C31B8C)]" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium text-white">Sessao Salva</div>
                        <div className="text-xs text-white/40 truncate">
                          {new Date(savedAudioMeta.generatedAt).toLocaleDateString('pt-BR')} •{' '}
                          {formatTime(savedAudioMeta.durationSeconds)}
                        </div>
                      </div>
                      <Button
                        size="sm"
                        onClick={loadSavedAudio}
                        disabled={loadingSavedAudio}
                        className="bg-white/10 border-white/10 text-white hover:bg-white/20 h-8 px-4"
                      >
                        {loadingSavedAudio ? <Loader2 className="w-3 h-3 animate-spin" /> : 'Carregar'}
                      </Button>
                    </div>
                  </div>
                )}
