...
| SlideDeckAgent | gemini-3-pro-preview + imagen | 0.7 | v4 batch-based slide generation |

### SlideDeck v4 Batch Architecture

The SlideDeck agent uses a **batch-based parallel architecture** to work within AgentCore's 120-second timeout limit while generating 8-15 professional slides per deck.

#### Problem Solved
Full slide deck generation takes 3-5 minutes, exceeding AgentCore's 120-second limit. Instead of reducing slide count, we split the work into batches.

#### Architecture
```
Frontend (React)
    │
    ▼ Phase 1: slidedeck_plan (~10-15s)
AgentCore → Extract concepts, create batch assignments
    │
    ▼ Phase 2: Parallel slidedeck_batch calls
AgentCore → Each batch generates 2-3 slides (~60-90s)
    │
    ▼ Frontend aggregates all batches
Complete slide deck (8-15 slides)
```

#### Endpoints
| Action | Timing | Description |
|--------|--------|-------------|
| `slidedeck_plan` | ~10-15s | Extract concepts and create batch plan |
| `slidedeck_batch` | ~60-90s | Generate 2-3 slides (cover, content, closing) |
| `generate_slidedeck` | LEGACY | May timeout for large decks |

#### Frontend Orchestration (`client/services/agentcore.ts`)
```typescript
...
The frontend:
1. Calls `slidedeck_plan` to get concept list and batch assignments
2. Makes parallel `slidedeck_batch` calls (cover, content batches, closing)
3. Aggregates results into final slide deck
4. Reports progress via callback for real-time UI updates
